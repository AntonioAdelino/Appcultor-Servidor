...
router.post('/', jsonParser, adaptRoute(new AddTagController(new TagGateway())));
/* atualiza um artigo especificando o id */
router.put('/:id', jsonParser, adaptRoute(new UpdateTagController(new TagGateway())));
/* remove um artigo especificando um id */
router.delete('/:id?', jsonParser, adaptRoute(new RemoveTagController(new TagGateway())));
